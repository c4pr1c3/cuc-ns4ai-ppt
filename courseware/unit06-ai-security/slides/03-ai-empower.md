...
* 代表：Security Copilot、Gemini for Security、Elastic ML

---

## SOAR：决策→行动（铁律）

* **SOAR**：响应剧本化（钓鱼邮件→提 IOC→查情报→命中则隔离+封禁）
...
* 漏洞：辅助分析反编译、生成 payload 变体（绕 WAF）
* **AI 代码审计**：静态分析器（高精度）+ LLM（高召回）**交叉验证**，修复须人工 review
...
* ✅ 赋能：自然语言查情报、生成检测规则
* ⚠️ 作为对象：库被投毒 → 输出被污染（slides 02 已述）
...
* **自动化评估工具**：[Promptfoo](https://www.promptfoo.dev/)（LLM 测试/红队）、DeepTeam（LLM/Agent 红队框架，50+ 漏洞类型）
...
* **国内自主 agent 标杆**：腾讯云智能渗透赛 TCH、[METATRON](https://github.com/sooryathejas/METATRON)（案例讨论，非教学依赖）
