...
* **LLM**：只输出文本
* **Agent**：调用工具、读写数据、执行动作（查库、发邮件、付款）
...
* **直接注入**：用户自己输入（越狱）
* **间接注入**：Agent **读取的第三方内容**（网页/邮件/文档/工具返回）
* Agent 时代主战场：攻击者**不接触用户**，只污染「Agent 会读的内容」
...
* Agent 互相委托 → 权限沿调用链**蔓延**
* A 信任 B，B 被劫持 → A 也被攻陷
* 编排层被注入 → 指挥全部子 Agent 作恶
