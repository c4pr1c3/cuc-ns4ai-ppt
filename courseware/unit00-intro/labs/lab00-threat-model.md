...
> 本 lab 即 capstone **M0**。你将从零立项，并对 fork 的种子应用完成第一份**威胁建模 + 风险登记表**。这是整学期工程的「地基」，后续 M1-M7 都在它之上演进。

## 1. 实验目标

- **能力交付（绑定簇①·L2）**:
  1. 能够 fork 种子工程并建立可演进的项目仓库骨架。
  2. 能够对一个具体 Web 应用做**资产清单 + CIA 标注**。
  3. 能够用 **STRIDE** 对系统威胁进行穷举式建模。
  4. 能够用 **CVSS 3.1** 对识别出的风险打分，并产出一份**风险登记表**。

> 完成本 lab 后，在里程碑报告中对照能力框架自评：点亮 **簇①·L1（认知）+ 簇①·L2（实践）**。
...
- **学术诚信**: 所有攻击实验**仅在你 fork 的靶场、授权环境**内进行；课程不教授编写恶意代码。本 lab 只做**建模与评分**，不写任何攻击 payload（攻击在 M3）。

> 种子应用 `app.py` 故意预留了攻击面（弱口令、SQL 拼接、Agent 端点），它们就是本次威胁建模的**真实素材**。

## 3. 任务清单

### 任务 A: 技术选型 + 仓库骨架

1. **fork 种子工程**到课程 GitLab，克隆到本地，确认 `python app.py` 可启动并访问 `/login`。
2. 在仓库根创建 `docs/` 目录，本 lab 的所有产物**提交到 `docs/m0/`** 下。
...
| Agent 查询输入 | 不可信输入 | `/api/agent` body | M | H | M | 注入入口 |
...
对 seed 应用的关键**组件/数据流**逐一过 STRIDE 六类，**不漏威胁**。建议建模对象至少包含：
...
| R3 | Agent 间接注入调用高危工具 | `AV:N/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:N` | 8.1 | 高 | 强化白名单 + 人在回路 + 输出过滤 | M6 |
...
在仓库 `docs/m0/` 下提交以下文件，并打 git tag `m0`：
...
| **工程规范** | 20 | 仓库可运行 + tag + 报告完整 | 基本规范 | 无法运行/无 tag |
...
| 簇 × 级 | 能力描述 | 是否点亮 | 证据 |
| :-: | :- | :-: | :- |
| ①·L1 | 复述 CIA/STRIDE/CVSS，识别资产与威胁 | ☐ | 课堂快问快答 + 本 lab 资产/威胁识别 |
